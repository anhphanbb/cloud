...
# # Preprocess Data: Stack grayscale image to create 3 channels
# data = data.map(lambda x, y: (tf.image.grayscale_to_rgb(x), y))
...
# Define Transfer Learning Model using ResNet-50
def create_resnet_model():
    base_model = ResNet50(weights='imagenet', include_top=False, input_tensor=Input(shape=(300, 900, 3)))
    base_model.trainable = False  # Freeze the base model layers

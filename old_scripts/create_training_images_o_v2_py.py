...
def save_radiance_as_images(nc_file_path, orbit_number, cloud_intervals, cloud_chance=0.15, clear_chance=0.06):
    with Dataset(nc_file_path, 'r') as nc:
        ...
        for i in range(5, radiance.shape[0]):
...
def save_image(data, index, folder, orbit_number, frame_index):
    ...
    # Normalize the previous frame radiance data
    prev_frame_5_norm = None
    if frame_index >= 5:
        prev_frame_5 = data[frame_index - 5]
        prev_frame_5_norm = np.clip((prev_frame_5 - min_radiance) / (max_radiance - min_radiance) * 255, 0, 255).astype(np.uint8)
    
    # Compute optical flow
    if prev_frame_5_norm is not None:
        h_channel, v_channel = compute_optical_flow(prev_frame_5_norm, norm_radiance)
    else:
        h_channel = np.zeros_like(norm_radiance, dtype=np.uint8)
        v_channel = np.zeros_like(norm_radiance, dtype=np.uint8)
    
    # Create a three-layer image
    three_layer_image = np.stack((norm_radiance, h_channel, v_channel), axis=-1)

...
box_center_lat = dataset.variables['Box_Center_Latitude'][:]  # Load box center latitude data
box_center_lon = dataset.variables['Box_Center_Longitude'][:]  # Load box center longitude data
...
def update_plot(time_step):
    ...
    for box_idx, (x_start, x_end, y_start, y_end) in enumerate(boxes):
        if mlcloud[current_time_step, box_idx] >= ml_threshold:
            ...
            if box_idx in [24, 25, 26]:
                cloud_center = True
                # Get the latitude and longitude of the center for the current time step
                center_lat = box_center_lat[current_time_step, box_idx]
                center_lon = box_center_lon[current_time_step, box_idx]
                # Append the center lat/lon information
                center_lat_lon_info += f"Box {box_idx}: Lat {center_lat:.2f}, Lon {center_lon:.2f}\n"
    ...
    # Display cloud center status, cloud presence count, and center lat/lon info if relevant
    ...
    # Display lat/lon info for the cloud center boxes if applicable
    if cloud_center:
        ax.text(0.02, 0.95, center_lat_lon_info, 
                verticalalignment='top', horizontalalignment='left',
                transform=ax.transAxes, color='blue', fontsize=10, fontweight='bold')
